...
function AntenatalHeroVisual() {
  return (
    <motion.div
      className="relative w-full overflow-hidden rounded-[1.8rem] border border-white/80 bg-white/55 p-3 text-[#1b2430] shadow-[inset_0_1px_0_rgba(255,255,255,.95),0_28px_90px_rgba(50,76,92,.14)] ring-1 ring-[#243446]/5 backdrop-blur-2xl sm:p-4"
      ...
      <div className="relative z-10">
        <div className="flex items-start justify-between gap-5">
          <div>
            ...
            <h2 className="mt-1 text-xl font-semibold tracking-[-0.04em] text-[#1b2430] sm:text-2xl">Your pregnancy timeline</h2>
            <p className="mt-1 text-xs text-[#5d6d7a]">Approved guidance, timed to your gestation.</p>
          </div>
          <motion.div
            className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[linear-gradient(145deg,#8b7bff,#6d5ed8)] text-center text-white shadow-[0_12px_34px_rgba(110,94,216,.3)]"
        ...
        <div className="relative mt-4 grid grid-cols-4 gap-2">
          ...
          {journey.map(({ week, label, detail, icon: Icon }, index) => (
            ...
            >
              ...
              <b className="mt-2 block text-xs text-[#1b2430]">Week {week}</b>
        ...
        <div className="mt-4 rounded-[1.25rem] border border-[#dce8e5] bg-[#f5fbf9]/90 p-3 shadow-[inset_0_1px_0_white]">
          ...
          <div className="mt-2 grid grid-cols-3 gap-2">
            {["Concern structured", "Clinical task created", "Midwife review"].map((item, index) => (
              <motion.div
                key={item}
                className="flex min-w-0 items-center gap-1.5 rounded-xl border border-white bg-white/80 px-2 py-2 text-[9px] font-medium text-[#526170] shadow-sm sm:text-[11px]"
          ...
          <div className="mt-2 flex items-center gap-2 text-[10px] font-medium text-[#6558d1]"><ShieldCheck size={14} />AI supports the workflow. Clinical decisions remain human.</div>
        </div>
      </div>
    </motion.div>
  );
  ...
  return (
    <Shell>
      <section className="mx-auto flex h-svh w-full max-w-7xl flex-col justify-center overflow-hidden px-5 pb-4 pt-16 text-center sm:px-8 lg:px-12">
        ...
        <motion.h1 className="mx-auto mt-2 max-w-5xl text-[clamp(2.2rem,6.5svh,4.5rem)] font-semibold leading-[0.98] tracking-[-0.06em] text-[#1b2430]" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.1 }}>Personalised guidance throughout the antenatal journey.</motion.h1>
        <motion.p className="mx-auto mt-2 max-w-2xl text-sm leading-6 text-[#526170]/86 sm:text-base" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, delay: 0.18 }}>Gestation-specific information, investigation schedules and approved education — with a governed pathway for raising clinical concerns.</motion.p>
