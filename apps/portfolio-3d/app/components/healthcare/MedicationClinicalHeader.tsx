...
export default function MedicationClinicalHeader() {
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      const focusable = Array.from(modalRef.current?.querySelectorAll<HTMLElement>("button:not([disabled]), select:not([disabled]), textarea:not([disabled]), [href], [tabindex]:not([tabindex='-1'])") ?? []);
  ...
  return <section className={styles.clinicalHeader}>
    <div className={styles.clinicalHeaderTop}><div><p className={styles.eyebrow}>Clinical Dashboard · Fictional records</p><h1>Medication verification queue</h1><p>The system detects and explains discrepancies. Healthcare professionals review and acknowledge the result.</p></div><button ref={acknowledgeButtonRef} className={styles.ackPrimary} onClick={() => setOpen(true)}>Acknowledge Review</button></div>
