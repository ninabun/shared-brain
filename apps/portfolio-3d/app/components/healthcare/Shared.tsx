...
export function ApplicationCard({ app, problem, solution, workflow, value, cta="Explore application", hideIcon=false }: { app:ApplicationConfig; problem:string; solution:string; workflow:string[]; value?:string; cta?:string; hideIcon?:boolean }) { return <article className={`${styles.card} ${styles.appCard}`}><div style={{display:"flex",justifyContent:"space-between",gap:12}}><SystemStatusBadge label="Interactive concept"/><SystemStatusBadge label={`${app.risk} risk`} tone="warn"/></div><h3>{app.title}</h3><p><b style={{color:"var(--ink)"}}>Clinical problem.</b> {problem}</p><p style={{marginTop:12}}><b style={{color:"var(--ink)"}}>Workflow response.</b> {solution}</p>{value&&<p style={{marginTop:12}}><b style={{color:"var(--ink)"}}>Value.</b> {value}</p>}<div className={styles.workflowMini}>{workflow.map(s=><span key={s}>{s}</span>)}</div><div className={styles.actions}><a href={`/projects/${app.slug}`} className={`${styles.button} ${styles.primary}`}>{cta} {!hideIcon&&<ArrowRight size={15}/>}</a></div></article>; }
